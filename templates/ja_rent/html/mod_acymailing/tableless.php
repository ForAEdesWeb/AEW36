...
<div class="row">
	...
	<div class="col-xs-12 col-sm-7 section-header">
		...
		<?php if(!empty($introText)): ?>
			<p class="container-sm block-intro"><?php echo $introText; ?></p>
		<?php endif; ?>	
	</div>
	<?php endif; ?>
				...
				<div class="acymailing_form">
						...
						foreach($fieldsToDisplay as $oneField){
							...
							if($oneField == 'name' AND empty($extraFields[$oneField])){
								...
								<span class="acyfield_<?php echo $oneField. (!$displayOutside? ' acy_requiredField':''); ?>"><input id="user_name_<?php echo $formName; ?>" <?php if(!empty($identifiedUser->userid)) echo 'readonly="readonly" '; if(!$displayOutside){ ?> onfocus="if(this.value == '<?php echo $nameCaption;?>') this.value = '';" onblur="if(this.value=='') this.value='<?php echo $nameCaption?>';"<?php } ?> class="inputbox" type="text" name="user[name]" style="width:<?php echo $fieldsize; ?>" value="<?php if(!empty($identifiedUser->userid)) echo $identifiedUser->name; elseif(!$displayOutside) echo $nameCaption; ?>" title="<?php echo $nameCaption;?>" placeholder="<?php echo JText::_( 'ENTER' ).' '.$nameCaption;?>"/></span>
								<?php
								...
								<span class="acyfield_<?php echo $oneField. (!$displayOutside? ' acy_requiredField':''); ?>"><input id="user_email_<?php echo $formName; ?>" <?php if(!empty($identifiedUser->userid)) echo 'readonly="readonly" '; if(!$displayOutside){ ?> onfocus="if(this.value == '<?php echo $emailCaption;?>') this.value = '';" onblur="if(this.value=='') this.value='<?php echo $emailCaption?>';"<?php } ?> class="inputbox" type="text" name="user[email]" style="width:<?php echo $fieldsize; ?>" value="<?php if(!empty($identifiedUser->userid)) echo $identifiedUser->email; elseif(!$displayOutside) echo $emailCaption; ?>" title="<?php echo $emailCaption;?>" placeholder="<?php echo JText::_( 'ENTER' ).' '.$emailCaption;?>" /></span>
								<?php
					...
					<?php }
					 ...
					 if($params->get('showterms',false)){
						...
						<p class="acysubbuttons">
							...
							<button class="button subbutton btn btn-lg btn-default btn-decor" onclick="try{ return submitacymailingform('optin','<?php echo $formName;?>'); }catch(err){alert('The form could not be submitted '+err);return false;}"><?php $subtext = $params->get('subscribetextreg'); if(empty($identifiedUser->userid) OR empty($subtext)){ $subtext = $params->get('subscribetext',JText::_('SUBSCRIBECAPTION')); } echo $subtext;  ?></button>
							...
							<button class="button unsubbutton btn btn-lg btn-inverse btn-decor" onclick="return submitacymailingform('optout','<?php echo $formName;?>')"><?php echo $params->get('unsubscribetext',JText::_('UNSUBSCRIBECAPTION')); ?></button>
							<?php } ?>
						</p>
					</div>
				<?php
