...
<div class="row">
	<?php if($specialTitle): ?>
	<div class="col-xs-12 <?php if(!$displayInline) echo 'col-sm-7'; ?> section-header">
	...
	<?php endif; ?>
	<div class="col-xs-12 <?php if(!$displayInline) echo 'col-sm-3'; ?> acymailing_module<?php echo $params->get('moduleclass_sfx')?>" id="acymailing_module_<?php echo $formName; ?>">
	<?php
		$style = array();
		...
		<div class="acymailing_fulldiv" id="acymailing_fulldiv_<?php echo $formName; ?>" <?php echo $styleString; ?> >
			...
			<div class="acymailing_module_form" >
				...
				?>
				<table class="acymailing_form <?php if($displayInline) echo 'acymailing_form_horizontal'; ?>">
					<tr>
					...
					if(empty($identifiedUser->userid) AND $config->get('captcha_enabled') AND acymailing_level(1)){ ?>
					 ...
					 if($params->get('showterms',false)){
						...
						<td <?php if($displayOutside AND !$displayInline) echo 'colspan="2"'; ?> class="acysubbuttons">
							...
							<button class="button subbutton btn btn-default btn-decor" onclick="try{ return submitacymailingform('optin','<?php echo $formName;?>'); }catch(err){alert('The form could not be submitted '+err);return false;}">
								<?php $subtext = $params->get('subscribetextreg'); if(empty($identifiedUser->userid) OR empty($subtext)){ $subtext = $params->get('subscribetext',JText::_('SUBSCRIBECAPTION')); } echo $subtext;  ?>
							</button>
							...
							<input class="button unsubbutton btn btn-inverse btn-decor" type="button" value="<?php echo $params->get('unsubscribetext',JText::_('UNSUBSCRIBECAPTION')); ?>" name="Submit" onclick="return submitacymailingform('optout','<?php echo $formName;?>')"/>
							<?php } ?>
						</td>
					</tr>
				</table>
